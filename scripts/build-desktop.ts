...
async function main() {
	...
	const targetPlatform = platformArg?.split('=')[1] ?? 'all';

	// Validate platform
	if (!['linux', 'win', 'all'].includes(targetPlatform)) {
		console.error('Invalid platform. Use: --platform=linux|win|all');
		process.exit(1);
	}

	const shouldBuildLinux = targetPlatform === 'linux' || targetPlatform === 'all';
	const shouldBuildWindows = targetPlatform === 'win' || targetPlatform === 'all';

	const platformsText = shouldBuildLinux && shouldBuildWindows
		? 'Linux and Windows'
		: shouldBuildLinux
			? 'Linux'
			: 'Windows';

	console.log(`Building desktop applications for ${platformsText}...\n`);
	...
	console.log('Building executables (this may take several minutes)...\n');
